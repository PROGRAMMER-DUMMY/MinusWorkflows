...
-1. **Global Sync (CRITICAL)**:
    - ALWAYS execute `uvx code-review-graph update` AND `node install.js` via the shell *before* attempting to answer the user, classify intent, or route the task. This ensures the structural graph and specialized skills are always synchronized with the latest codebase state.
