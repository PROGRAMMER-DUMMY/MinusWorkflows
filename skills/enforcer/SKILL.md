...
2. **Snapshot Retrieval (Linear History)**:
   ...
   - Load only the JSON snapshots from `.memory/snapshots/` that contain structural changes relevant to those modules.
3. **Skill Unloading**: Once a phase is marked `Done`, explicitly "Unload" those instructions.
